...
contract("WyvernExchange", (accounts) => {
  ...
  it("correctly hashes order to sign", async () => {
    ...
    assert.equal(
      hashToSign(example, exchange.inst.address),
      hash,
  ...
  it("does not allow set-fill to same fill", async () => {
    ...
    let example = {
      registry: registry.address,
      maker: accounts[1],
      staticTarget: exchange.inst.address,
      staticSelector: "0x00000000",
      staticExtradata: "0x",
      maximumFill: "1",
      listingTime: "0",
  ...
  it("validates valid authorization by signature (sign_typed_data)", async () => {
    ...
    assert.isTrue(
      await exchange.validateOrderAuthorization(hash, accounts[1], signature, {
        from: accounts[5],
      }),
      "Should have validated",
    );
  ...
  it("does not validate authorization by signature with different prefix (personal_sign)", async () => {
    ...
    let example = {
      registry: registry.address,
      maker: accounts[1],
      staticTarget: wrappedExchange.inst.address,
      staticSelector: "0x00000000",
      staticExtradata: "0x",
      maximumFill: "1",
      listingTime: "0",
  ...
  it("validates valid authorization by approval", async () => {
    ...
    let valid = await exchange.validateOrderAuthorization(hash, accounts[0], {
      v: 27,
      r: ZERO_BYTES32,
      s: ZERO_BYTES32,
    });
  ...
  it("does not validate authorization without signature", async () => {
    ...
    const hash = hashOrder(example);
    let valid = await exchange.validateOrderAuthorization(hash, accounts[1], {
      v: 27,
      r: ZERO_BYTES32,
      s: ZERO_BYTES32,
    });
